...
from FastAutoAugment.archive import (
    arsaug_policy,
    autoaug_policy,
...
from FastAutoAugment.augmentations import Lighting, apply_augment
...
def get_dataloaders(
    ...
    if "cifar" in dataset or "svhn" in dataset:
        ...
        transform_train = transforms.Compose(
            [
                ...
                # transforms.RandomResizedCrop(input_size, scale=(0.1, 1.0),
        ...
        raise ValueError("No transforms for dataset=%s" % dataset)

    total_aug = augs = None
    if isinstance(C.get()["aug"], list):
        logger.debug("Num augmentations provided: %s", len(C.get()["aug"]))
        ...
        if C.get()["aug"] == "fa_reduced_cifar10":
            ...
            raise ValueError("Augmentations not found: %s" % C.get()["aug"])
    ...
    if dataset == "cifar10":
        ...
        trainset = torchvision.datasets.SVHN(
            root=dataroot, split="train", download=True, transform=transform_train
        )
        extraset = torchvision.datasets.SVHN(
            root=dataroot, split="extra", download=True, transform=transform_train
        )
        ...
        total_trainset = torchvision.datasets.SVHN(
            root=dataroot, split="train", download=True, transform=transform_train
        )
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=73257 - 1000, random_state=0
        )  # 1000 trainset
        sss = sss.split(list(range(len(total_trainset))), total_trainset.targets)
        train_idx, valid_idx = next(sss)
        targets = [total_trainset.targets[idx] for idx in train_idx]
        ...
        total_trainset = ImageNet(
            root=os.path.join(dataroot, "imagenet-pytorch"), transform=transform_train
        )
        testset = ImageNet(
        ...
        # fmt: off
        idx120 = [
            16, 23, 52, 57, 76, 93, 95, 96, 99, 121, 122, 128, 148, 172, 181, 189, 202,
            210, 232, 238, 257, 258, 259, 277, 283, 289, 295, 304, 307, 318, 322, 331,
            337, 338, 345, 350, 361, 375, 376, 381, 388, 399, 401, 408, 424, 431, 432,
            440, 447, 462, 464, 472, 483, 497, 506, 512, 530, 541, 553, 554, 557, 564,
            570, 584, 612, 614, 619, 626, 631, 632, 650, 657, 658, 660, 674, 675, 680,
            682, 691, 695, 699, 711, 734, 736, 741, 754, 757, 764, 769, 770, 780, 781,
            787, 797, 799, 811, 822, 829, 830, 835, 837, 842, 843, 845, 873, 883, 897,
            900, 902, 905, 913, 920, 925, 937, 938, 940, 941, 944, 949, 959,
        ]
        # fmt: on
        assert len(idx120) == len(set(idx120)) == 120
        total_trainset = ImageNet(
            root=os.path.join(dataroot, "imagenet-pytorch"), transform=transform_train
        )
        testset = ImageNet(
        ...
        sss = StratifiedShuffleSplit(
            n_splits=1, test_size=len(total_trainset) - 50000, random_state=0
        )  # 4000 trainset
        sss = sss.split(list(range(len(total_trainset))), total_trainset.targets)
        train_idx, valid_idx = next(sss)
        ...
        test_idx = list(
            filter(lambda x: testset.samples[x][1] in idx120, range(len(testset)))
        )
        ...
        for idx in range(len(total_trainset.samples)):
            if total_trainset.samples[idx][1] not in idx120:
                continue
            total_trainset.samples[idx] = (
                total_trainset.samples[idx][0],
                idx120.index(total_trainset.samples[idx][1]),
            )
        total_trainset = Subset(total_trainset, train_idx)
        total_trainset.targets = targets
    ...
    logger.debug("total_trainset=%s, testset=%s", len(total_trainset), len(testset))
    ...
    if split > 0.0:
        ...
        for _ in range(split_idx + 1):
            train_idx, valid_idx = next(sss)
        logger.debug(
            "split_idx %s: len(train)=%s, len(val)=%s",
            split_idx,
            len(train_idx),
            len(valid_idx),
        )
        ...
        if multinode:
            train_sampler = torch.utils.data.distributed.DistributedSampler(
                total_trainset, num_replicas=dist.get_world_size(), rank=dist.get_rank()
            )
            logger.info(
                "----- dataset with DistributedSampler %s / %s",
                dist.get_rank(),
                dist.get_world_size(),
            )
...
class EfficientNetCenterCrop:
    ...
    def __call__(self, img):
        """Crop the given PIL Image to a square image.

        Args:
            img (PIL Image): Image to be cropped.
            imgsize (int): defines the output size.
            output_size = crop_size = imgsize / (imgsize + 32) * min(img.size)
        Returns:
